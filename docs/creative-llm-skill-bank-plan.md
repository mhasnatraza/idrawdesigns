...
## 10) Language-light capture mode (non-English friendly)
W2 supports symbolic capture so entries do not need full English prose.
- Use the `mixel` iconographic token format for compact records.
- Store micro observations as symbols, then roll up to macro and macro^2 trend maps.
- Keep optional text expansions only when needed for sharing.

See `docs/iconographic-encoding-protocol.md` for the full token system.
